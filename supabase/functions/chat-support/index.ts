...
serve(async (req) => {
  ...
  try {
    ...
    // Handle human request action
    if (action === 'request_human') {
      if (!conversationId) {
        throw new Error('Conversation ID required');
      }

      // Update conversation to mark human requested
      ...
      try {
        ...
        if (RESEND_API_KEY) {
          const emailHtml = `
            <h2>Human Agent Requested in Chat</h2>
            <p><strong>Conversation ID:</strong> ${conversationId}</p>
            <p><strong>User:</strong> ${userName || userEmail || 'Anonymous'}</p>
            <p><strong>Email:</strong> ${userEmail || 'Not provided'}</p>
            <p><strong>Requested at:</strong> ${new Date().toLocaleString()}</p>
            <hr>
            <p><a href="https://app.disclosurely.com/dashboard/admin/chat">View Conversation in Admin Panel</a></p>
          `;
          ...
          await fetch('https://api.resend.com/emails', {
            ...
            body: JSON.stringify({
              from: 'Disclosurely Chat <notifications@disclosurely.com>',
              to: ['sampettiford@googlemail.com'],
              subject: `Human Agent Requested - Chat ${conversationId.substring(0, 8)}`,
              html: emailHtml
            })
          });
        }
