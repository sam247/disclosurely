...
async function createContentfulEntry(blogData, authorId, categoryId) {
  ...
  console.log('✅ Contentful entry created successfully!');
...
async function main() {
  ...
  if (!authorId) {
    console.error(`❌ Author "${authorName}" not found. Available authors: ${Object.keys(AUTHORS).join(', ')}`);
    process.exit(1);
  }
  
  if (!categoryId) {
    console.error(`❌ Category "${categoryName}" not found. Available categories: ${Object.keys(CATEGORIES).join(', ')}`);
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(`⏱️ Reading time: ${blogData.readingTime} minutes`);
